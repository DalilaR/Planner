...
class PlanningGraph():
    ...
    def create_graph(self):
        ...
        while not leveled:
            ...
            t = self.a_levels[level]
            self.update_a_mutex(t)
    ...
    def add_action_level(self, level):
        ...
        #In here, for each possible action, get its prenodes and see if all the prenodes of an action are in S[level].
        #As we are working with sets, a use of the function issubset is more appropriate than using loops to make sure that
        # all the prenodes of an action are indeed in S[level]
        for action in self.all_actions:
            x = PgNode_a(action).prenodes
            l_x = len(x)
            s = self.s_levels[level]
            if x.issubset(s):
                pass_test = True
            else:
                pass_test = False
            if pass_test:
                #make sure to create an action node, and to populate its parents.
                #finally, make sure to add the action node to the graph
                y = PgNode_a(action)
                if y.action.name != 'Noop_pos' and y.action.name != 'Noop_neg':
                    y.parents.update(x)
                self.a_levels[level].add(y)
    def add_literal_level(self, level):
        ...
        for action in self.a_levels[level-1]:
            tmp = action.effnodes
            for a in action.effnodes:
                action.children.add(a)
            for x in tmp:
                x.parents.add(action)
            self.s_levels[level].update(tmp)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        '''inconsistent = False
        for a1 in node_a1.effnodes:
           for a2 in node_a2.effnodes:
                if a1.symbol == a2.symbol and a1.is_pos != a2.is_pos:
                    return True
        return False'''
        a1_effects = node_a1.effnodes
        a2_effects = node_a2.effnodes

        for a1_effnode in a1_effects:
            for a2_effnode in a2_effects:
                if (a1_effnode.symbol == a2_effnode.symbol) and (a1_effnode.is_pos != a2_effnode.is_pos):
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        Way 1: inconsistent = False
        for a1 in node_a1.effnodes:
            for a2 in node_a2.prenodes:
                if a1.symbol == a2.symbol and a1.is_pos != a2.is_pos:
                    inconsistent = True
                    return inconsistent
        for a1 in node_a1.prenodes:
            for a2 in node_a2.effnodes:
                if a1.symbol == a2.symbol and a1.is_pos != a2.is_pos:
                    inconsistent = True
                    return inconsistent
        return inconsistent.
        The second way, was shown to me by my corrector'''
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # I did what I was asked for, but unitest insist that it is wrong
        a1_prenodes = node_a1.prenodes
        a2_prenodes = node_a2.prenodes
        #node_a1.show()
        for a1 in node_a1.parents:
            for a2 in node_a2.parents :
                if a1.is_mutex(a2):
                    ...
                    if (a1.is_pos != a2.is_pos) and (a1.symbol == a2.symbol):
                        return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        '''This took me a long time to understand and code.  After, I was able to figure out what comes 
        first, I was to implement it.
        In here we really have 3 cases:
        Case 1:  if the states are negative of each others: At(C1, P1) and Not At(C1, P1), that they
         are mutext
        Case 2:  if an action has both node_s1 and node_s2, than node_s1 and node_s2 are not mutex
        Case 3, final case: if any two parents (or actions) are mutex, then node_s1 and node_s2 are
        mutex'''
        tmp = set([node_s1,node_s2])
        if (node_s1.symbol == node_s2.symbol) and (node_s1.is_pos != node_s2.is_pos):
            return True
        for p1 in node_s1.parents:
            if len(p1.effnodes.intersection(tmp)) == 2:
                return False
        for p2 in node_s2.parents:
            if len(p2.effnodes.intersection(tmp)) == 2:
                return False
        for p1 in node_s1.parents:
            for p2 in node_s2.parents:
                if p1.is_mutex(p2):
...
<<<<<<< HEAD
        ...
        for g in self.problem.goal:
            ...
            l = 0
            while not stop:
                for k in self.s_levels[l]:
                    if k.is_pos and k.symbol == g:
                        tmp_length += l
                        found = True
                        stop = True
                        break
